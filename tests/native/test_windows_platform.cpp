...
TEST_CASE("Native picker polls report the active tool", "[native][picker-mode]")
{
    for (const auto mode : {RegionPickerMode::DrawGlobal, RegionPickerMode::AttachWindow, RegionPickerMode::AttachFace,
                            RegionPickerMode::PinColor}) {
        PickerState first;
        PickerState second;
        first.drawMode = mode == RegionPickerMode::DrawGlobal;
        first.facesMode = mode == RegionPickerMode::AttachFace;
        first.pinMode = mode == RegionPickerMode::PinColor;
...
TEST_CASE("Face detection completes from either caller apartment", "[native][face-detection]")
{
    ...
    for (const auto apartment : {winrt::apartment_type::single_threaded, winrt::apartment_type::multi_threaded}) {
        FaceDetectionResult result;
        {
            const std::jthread caller([&] {
                winrt::init_apartment(apartment);
                result = detectFaces(frame, 1.0f);
