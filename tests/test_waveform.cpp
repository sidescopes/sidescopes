...
TEST_CASE("Waveform attenuates a display-pipeline pileup")
{
    ...
    // code and starve its neighbors - here level 100 carries twenty-one
    // rows for every one of its neighbors, next to a hole at 102. A real
    // photo feature cannot look like this; the pileup must come down to
    // its neighborhood instead of rendering as a bright line.
    TestFrame frame(64, 450, 255);
    int pileupRows = 0;
    int neighborRows = 0;
    for (int py = 0; py < frame.height; ++py) {
        const int slot = py % 45;
        uint8_t value = static_cast<uint8_t>(slot < 26 ? 90 + slot : 100);
        if (value == 102) {
            value = 100;  // and 102 is never emitted
        }
        pileupRows += value == 100 ? 1 : 0;
        neighborRows += value == 97 ? 1 : 0;
        frame.fillRows(py, py + 1, Color{value, value, value});
    }
    REQUIRE(pileupRows == neighborRows * 21);
