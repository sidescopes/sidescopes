...
TEST_CASE("Only the keys a descriptor declares are handed to its scope")
{
    ...
    REQUIRE(assembled.size() == scope->descriptor->param_count);
    for (const SsParamValue& parameter : assembled) {
        CHECK(std::string(parameter.key) != "response");
        CHECK(std::string(parameter.key) != "smoothing_ms");
        if (std::string(parameter.key) == "gamma") {
            CHECK(parameter.value == 0.9);
        }
    }
}
