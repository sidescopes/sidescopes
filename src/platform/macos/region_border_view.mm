...
- (unsigned)zoneAtPoint:(NSPoint)point
{
    ...
    }
    {
        ...
        const CGFloat dx = point.x - center.x;
        const CGFloat dy = point.y - center.y;
        if (dx * dx + dy * dy <= sidescopes::BindingHitRadius * sidescopes::BindingHitRadius) {
...
- (void)mouseDown:(NSEvent*)event
{
    ...
    // Key without activation: the click focuses the region, so application
    // shortcuts work right after a border interaction, and the
