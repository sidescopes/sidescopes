...
void WarmFaceDetection() {
    ...
    dispatch_async(dispatch_get_global_queue(QOS_CLASS_UTILITY, 0), ^{
      ...
      if (CVPixelBufferCreateWithBytes(kCFAllocatorDefault, 64, 64, kCVPixelFormatType_32BGRA,
                                       blank, 64 * 4, nullptr, nullptr, nullptr,
...
std::vector<IntRect> DetectFaces(const FrameView& frame, float pixels_per_point) {
    ...
    if (performed && !error) {
        ...
        for (VNFaceObservation* observation in request.results) {
            ...
            rect.width = static_cast<int>(
                std::lround(std::min(width, static_cast<double>(frame.width) - rect.x)));
            rect.height = static_cast<int>(
                std::lround(std::min(height, static_cast<double>(frame.height) - rect.y)));
