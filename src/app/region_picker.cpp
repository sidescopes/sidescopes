...
// The border-free wait reads the wall clock, and a finished display scan wakes
// the main loop's idle wait so it is drained the moment it lands. Both are
// declared rather than pulling in the whole GLFW header: this unit compiles
// into the test binary too, which links no windowing library and provides its
// own stubs.
...
RegionPicker::RegionPicker(CaptureController& capture, AnalysisWorker& worker, ScreenCaptureSource& source)
    : m_capture(capture),
      m_worker(worker),
      m_source(source)
{
...
void RegionPicker::request(RegionPickerMode mode)
{
    m_want = mode;
}

void RegionPicker::clearRequest()
{
    m_want.reset();
}
...
void RegionPicker::openRegionPicker(RegionPickerMode mode, bool regionIsFullScreen)
{
    ...
    if (beginRegionPick(pickerDisplays, mode)) {
        m_picking = true;
        m_isPin = mode == RegionPickerMode::PinColor;
...
std::vector<PickerDisplay> RegionPicker::buildPickerDisplays()
{
    ...
    for (const CaptureTarget& target : m_source.listTargets()) {
        PickerDisplay entry;
...
// open.
void RegionPicker::launchDisplayFaceScans(const std::vector<PickerDisplay>& pickerDisplays)
{
    if (!supportsFaceDetection()) {
        return;
    }
...
void RegionPicker::drainFaceScans()
{
...
void RegionPicker::consumeDisplayFaceScan(DisplayFaceScan& scan)
{
    ...
    if (!m_picking || scan.generation != m_facePickGeneration) {
        return;
    }
...
const RegionPicker::WindowCandidate* RegionPicker::matchWindowCandidate(uint32_t displayId,
                                                                        const RegionOfInterest& region) const
{
...
// pick attaches to.
const RegionPicker::WindowCandidate* RegionPicker::windowContaining(uint32_t displayId,
                                                                    const RegionOfInterest& region) const
{
...
const FaceCandidate* RegionPicker::matchFaceCandidate(uint32_t displayId, const RegionOfInterest& region) const
{
    constexpr double MatchTolerance = 1.0;
                                     ...
                                     std::optional<FloatColor> screenSampleColor)
{
    ...
    return processPoll(pollRegionPick(), frameSize, screenSampleColor);
}

RegionPickOutcome RegionPicker::processPoll(const RegionPickPoll& poll,
                                            ...
                                            std::optional<FloatColor> screenSampleColor)
{
    ...
    return processRegionPoll(poll);
}

RegionPickOutcome RegionPicker::processPinPoll(const RegionPickPoll& poll,
                                               std::optional<AnalysisWorker::FrameSize> frameSize,
                                               std::optional<FloatColor> screenSampleColor)
{
    RegionPickOutcome outcome;
...
RegionPickOutcome RegionPicker::processRegionPoll(const RegionPickPoll& poll)
{
    ...
    if (poll.preview && poll.displayId == m_capture.capturedDisplay()) {
        outcome.previewRegion = *poll.preview;
    }
    if (poll.finished || !poll.active) {
        m_picking = false;
        if (poll.confirmed) {
