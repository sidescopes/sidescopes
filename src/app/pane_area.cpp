...
void PaneArea::drawScopeById(std::string_view id, Pass& pass)
{
    ...
    if (id == VectorscopeScopeId) {
        ...
        if (instance != nullptr) {
            drawHistogram(
                textureForId(id), pass.input.regionSelected, outlineFor(id), *instance, histogramStyle(),
                graticuleStyle(DefaultLineWidth),
                m_view.cursorMarkersVisible() && pass.input.regionSelected ? pass.input.vectorscopeColor : std::nullopt,
                m_histogramScratch);
        }
...
void PaneArea::drawVectorscopePane(Pass& pass)
{
    ...
    if (instance != nullptr) {
        ...
        if (m_view.cursorMarkersVisible() && pass.input.regionSelected && pass.input.vectorscopeColor) {
...
void PaneArea::drawWaveformPane(std::string_view id, Pass& pass)
{
    ...
    if (instance != nullptr) {
        ...
        if (m_view.cursorMarkersVisible() && pass.input.regionSelected && pass.input.waveformColor) {
