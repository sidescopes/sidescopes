...
constexpr NormalizedPoint kCenter{0.5f, 0.5f};

// The reference skin color whose projection anchors the skin-tone line.
constexpr FloatColor kSkinToneReference{203.0f, 171.0f, 153.0f};
...
constexpr BarColor kBarColors[6] = {
...
VectorscopeGraticule BuildVectorscopeGraticule(const Vectorscope& scope) {
    ...
    graticule.circles.push_back({kCenter, 0.25f, GraticuleStroke::Grid});
    graticule.circles.push_back({kCenter, 0.5f, GraticuleStroke::Grid});
    for (int tick = 0; tick < 12; ++tick) {
        const float angle = static_cast<float>(tick) * 3.14159265f / 6.0f;
        const float dx = std::cos(angle);
        const float dy = std::sin(angle);
        graticule.lines.push_back({{0.5f + dx * 0.48f, 0.5f + dy * 0.48f},
                                   {0.5f + dx * 0.5f, 0.5f + dy * 0.5f},
                                   GraticuleStroke::GridMajor});
    }

    // Color targets at 75% (primary, labeled) and 100% (secondary).
    for (const BarColor& bar : kBarColors) {
        if (const auto at75 = scope.Project(bar.color)) {
            graticule.targets.push_back({*at75, true, bar.label});
        }
        const FloatColor full{bar.color.r > 0.0f ? 255.0f : 0.0f,
                              bar.color.g > 0.0f ? 255.0f : 0.0f,
    ...
    if (const auto skin = scope.Project(kSkinToneReference)) {
        const float dx = skin->x - kCenter.x;
        const float dy = skin->y - kCenter.y;
        const float length = std::sqrt(dx * dx + dy * dy);
        if (length > 1e-6f) {
            const float scale = 0.5f / length;
            graticule.lines.push_back({kCenter,
                                       {kCenter.x + dx * scale, kCenter.y + dy * scale},
                                       GraticuleStroke::SkinTone});
        }
    }
...
std::vector<WaveformScaleLine> BuildWaveformScale() {
    ...
    for (int step = 0; step <= 10; ++step) {
        ...
        line.level_percent = static_cast<float>(100 - step * 10);
