...
enum class ColorSpaceHint { Unknown, Srgb, DisplayP3, DisplayProfile };
...
struct IntRect {
    ...
    [[nodiscard]] IntRect ClampedTo(int frame_width, int frame_height) const;
};
...
struct FrameView {
    ...
    [[nodiscard]] const uint8_t* PixelAt(int px, int py) const {
        return bgra + static_cast<std::size_t>(py) * stride_bytes +
